...
public static class AsyncTools
{
	...
	public static void WhereAmI(string text)
	{
		if (IsMainThread())
		{
            ...
            UnityEngine.Debug.LogWarning($"{text}: main thread, {contextName}, frame: {Time.frameCount}");
		}
		else
		{
			UnityEngine.Debug.LogWarning($"{text}: background thread, id: {Thread.CurrentThread.ManagedThreadId}");
		}
	}
